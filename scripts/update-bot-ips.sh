...
update_ols_config() {
    ...
    # Instead of creating a separate file, update the main security config
    local main_config="/usr/local/lsws/conf.d/wordpress-security.conf"

    if [[ -f "$main_config" ]]; then
        # Create backup of main config
        sudo cp "$main_config" "${main_config}.backup.$(date +%Y%m%d_%H%M%S)"

        # Update the Googlebot and Bingbot verification section
        local temp_config="$TEMP_DIR/updated-security.conf"

        # Copy everything up to the bot verification section
        sed '/# === GOOGLEBOT VERIFICATION (SIMPLIFIED) ===/,$d' "$main_config" > "$temp_config"

        # Add updated bot verification rules
        cat >> "$temp_config" << EOF

    # === GOOGLEBOT VERIFICATION (DYNAMIC) ===
    # Only verify if user agent claims to be Googlebot
    RewriteCond %{HTTP_USER_AGENT} ^googlebot [NC]
EOF

        # Add Google IP ranges (limit to 20 for performance)
        if [[ -f "$GOOGLE_IP_FILE" ]]; then
            local count=0
            while read -r ip_pattern && [[ $count -lt 20 ]]; do
                if [[ -n "$ip_pattern" ]]; then
                    echo "    RewriteCond %{REMOTE_ADDR} $ip_pattern [OR]" >> "$temp_config"
                    ((count++))
                fi
            done < "$GOOGLE_IP_FILE"

            # Remove last OR and add final rule
            sed -i '$ s/ \[OR\]$//' "$temp_config"
            echo "    RewriteRule ^ - [F,L]" >> "$temp_config"
        else
            echo "    RewriteCond %{REMOTE_ADDR} !^66\\.249\\." >> "$temp_config"
            echo "    RewriteRule ^ - [F,L]" >> "$temp_config"
        fi

        # Add Bingbot verification
        cat >> "$temp_config" << EOF

    # === BINGBOT VERIFICATION (DYNAMIC) ===
    # Only verify if user agent claims to be Bingbot
    RewriteCond %{HTTP_USER_AGENT} ^bingbot [NC]
EOF

        # Add Bing IP ranges (limit to 10 for performance)
        if [[ -f "$BING_IP_FILE" ]]; then
            local count=0
            while read -r ip_pattern && [[ $count -lt 10 ]]; do
                if [[ -n "$ip_pattern" ]]; then
                    echo "    RewriteCond %{REMOTE_ADDR} $ip_pattern [OR]" >> "$temp_config"
                    ((count++))
                fi
            ...
            echo "    RewriteCond %{REMOTE_ADDR} !^157\\.55\\." >> "$temp_config"
            echo "    RewriteRule ^ - [F,L]" >> "$temp_config"
        fi

        # Add the rest of the original config
        sed -n '/# Note: For dynamic IP updates/,$p' "$main_config" >> "$temp_config"

        # Install the updated configuration
        sudo mv "$temp_config" "$main_config"
        sudo chmod 644 "$main_config"

        print_status "OpenLiteSpeed configuration updated with dynamic bot verification"
    else
        print_warning "Main security configuration not found at $main_config"
    fi
}
